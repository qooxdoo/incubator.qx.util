 ...
 */
qx.Class.define("qx.util.AbstractConfig", {
  ...
  construct: function(config) {
    ...
    for (let prop of ["fileName", "version", "schemaBaseUrl"]) {
      if (!this.get(prop)) {
  ...
  members: {
    ...
    _validateData(data) {
      ...
      try {
        qx.util.Json.validate(data, this.__schema);
      } catch (e) {
     ...
     */
    getSchemaPath() {
      return path.join("schema", this._getSchemaFileName());
    },
     ...
     */
    getSchemaUri() {
      return qx.util.AbstractConfig.schemaBaseUrl + "/" + this._getSchemaFileName();
    },
     ...
     * @return {qx.util.AbstractConfig} Returns the instance for chaining
     */
    async load(data=undefined) {
      if (data === undefined) {
        ...
        if (await fs.existsAsync(this.getDataPath())) {
          // load data from file
          data = qx.util.Json.parseJson(await fs.readFileAsync(this.getDataPath(), "utf8"));
        } else if (this.isCreateIfNotExists()) {
          // we're supposed to create it, 
      ...
      if (this.isValidate() && this.getVersion() !== null) {
        if (!this.__schema) {
          ...
          this.__schema = await qx.util.Json.loadJsonAsync(this.getSchemaPath());
        }
        // check initial data
        let dataSchemaInfo = qx.util.Json.getSchemaInfo(data);
        if (!dataSchemaInfo) {
    ...
    async save() {
      this.validate();
      await qx.util.Json.saveJsonAsync(this.getDataPath(), this.getData());
    }
  }
});
